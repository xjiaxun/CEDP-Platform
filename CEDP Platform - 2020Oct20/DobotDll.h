...
extern "C" int ConnectDobot(const char *portName, uint32_t baudrate, char *fwType, char *version, float *time);
extern "C" int DisconnectDobot(void);
extern "C" int GetMarlinVersion(void);
...
extern "C" int GetDeviceVersion(uint8_t *majorVersion, uint8_t *minorVersion, uint8_t *revision, uint8_t *hwVersion);

extern "C" int SetDeviceWithL(bool isWithL, uint8_t version, bool isQueued, uint64_t *queuedCmdIndex);
extern "C" int GetDeviceWithL(bool *isWithL);

extern "C" int GetDeviceTime(uint32_t *deviceTime);
extern "C" int GetDeviceInfo(DeviceCountInfo *deviceInfo);
...
extern "C" int SetColorSensor(bool enable, ColorPort colorPort, uint8_t version);
extern "C" int GetColorSensor(uint8_t *r, uint8_t *g, uint8_t *b);

extern "C" int SetInfraredSensor(bool enable, InfraredPort infraredPort, uint8_t version);
...
//LOSTSTEP
extern "C" int SetLostStepParams(float threshold, bool isQueued, uint64_t *queuedCmdIndex);
...
extern "C" int SetQueuedCmdStartDownload(uint32_t totalLoop, uint32_t linePerLoop,uint32_t index);
...
extern "C" int GetQueuedCmdMotionFinish(bool *isFinish);
//parker add 设置debug输出使能
extern "C" bool SetDebugEnable(bool flag = false);
#endif // DOBOTDLL_H
